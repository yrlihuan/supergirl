...
- (void) saveKeywordRangeOfText:(NSString *)keyWord
{
     ...
     for (int i = 0  ; i < nLen; i++) 
    {
        ...
        //获取单个关键字符在text中的位置
        //NSString *strText = self.text;
        NSString *strText = self.keystring;
        NSRange range = [strText rangeOfString:keyWord];
        //由于结构体不能直接存到NSArray中，所以要转换一下。
        NSValue *value = [NSValue valueWithRange:range];
...
//    [mutaString addAttribute:(NSString *)kCTStrokeWidthAttributeName value:(id)cfNum2 range:NSMakeRange(0, len)];
    //段落
    CTParagraphStyleRef pa=CTParagraphStyleCreate(NULL, 0); 
    [mutaString addAttribute:(NSString *)kCTParagraphStyleAttributeName value:(id)pa range:NSMakeRange(0, len)];
...
- (void)drawRect:(CGRect)rect 
{
	...
	CTLineRef line = CTLineCreateWithAttributedString((CFAttributedStringRef) 
                                                      [self illuminatedString:self.keystring font:self.font]);	//[UIFont fontWithName:fontName size:60]
	...
	CGContextSetTextPosition(context, 160-[self.keystring length]*0.5*sizeof(self.font), 0.0); /*ceill(self.bounds.size.height) + 8*/
    //在离屏上绘制line
